 ...
 * 出勤簿（月次）の集計。
 *
 * 紙の出勤簿と同じ行データ・合計・サマリーを組み立てる純粋関数だけを置く。
 * **react-pdf にも DOM API にも依存しない**ので、ブラウザ側の PDF/Excel 出力からも、
 * サーバー側の API ルートからも同じ集計を使える。
 *
 * 元は utils/attendanceMonthlyPdf.tsx（'use client' かつ @react-pdf/renderer を import）
 * の中にあったが、人事システム(yushin-hr)へ出勤簿を渡す外部APIをサーバーで動かすために
 * 切り出した。集計式は MonthlyAttendanceView の aggregates と揃えてある。
 *
 * 型はPDFコンポーネント側の定義をそのまま使う（**型のみの import なので実行時には残らない**）。
 */
import type {
    AttendanceCellKind,
    AttendanceMonthlyPdfDay,
    AttendanceMonthlyPdfSummary,
    AttendanceMonthlyPdfTotals,
