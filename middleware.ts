...
export const config = {
    matcher: [
        ...
        // - api/cron: Vercel Cron からの定期実行
        //   (NextAuthトークンを持たないため middleware の 401 に阻まれていた。
        //    認証は route 側の Authorization: Bearer CRON_SECRET が担う)
        ...
        '/((?!api/auth|api/init-db|api/health|api/line/webhook|api/cron|api/external|login|robots\\.txt|_next/static|_next/image|favicon.ico|manifest\\.json|.*\\.png|.*\\.jpg|.*\\.svg|.*\\.ico|.*\\.mjs).*)',
    ],
};
