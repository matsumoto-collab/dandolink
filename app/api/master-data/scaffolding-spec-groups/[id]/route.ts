...
export async function PATCH(request: NextRequest, context: RouteContext) {
    try {
        ...
        const group = await prisma.scaffoldingSpecGroup.update({ where: { id }, data: updateData });
        return NextResponse.json(group);
    } catch (error) {
        return serverErrorResponse('足場仕様グループ更新', error);
    }
}
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
    try {
        ...
        const { id } = await context.params;
        await prisma.scaffoldingSpecGroup.update({ where: { id }, data: { isActive: false } });
        return NextResponse.json({ success: true });
    } catch (error) {
        return serverErrorResponse('足場仕様グループ削除', error);
    }
}
