    ...
    try {
        ...
        if (name !== undefined) {
            ...
            if (name.trim().length > 200) {
                return validationErrorResponse('名前は200文字以内で入力してください');
            }
        ...
        const title = await prisma.invoiceTitleSuggestion.update({
            where: { id },
            data: updateData,
        });
        return NextResponse.json(title);
    } catch (error) {
        return serverErrorResponse('請求書タイトル候補更新', error);
    }
}
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
    try {
        ...
        const { id } = await context.params;
        await prisma.invoiceTitleSuggestion.update({
            where: { id },
            data: { isActive: false },
        });
        return NextResponse.json({ success: true });
    } catch (error) {
        return serverErrorResponse('請求書タイトル候補削除', error);
    }
}
