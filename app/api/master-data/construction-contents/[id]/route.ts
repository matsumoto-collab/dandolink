...
export async function PATCH(request: NextRequest, context: RouteContext) {
    try {
        ...
        const content = await prisma.constructionContent.update({
            where: { id },
            data: updateData,
        });
        return NextResponse.json(content);
    } catch (error) {
        return serverErrorResponse('工事内容更新', error);
    }
}
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
    try {
        ...
        const { id } = await context.params;
        await prisma.constructionContent.update({
            where: { id },
            data: { isActive: false },
        });
        return NextResponse.json({ success: true });
    } catch (error) {
        return serverErrorResponse('工事内容削除', error);
    }
}
