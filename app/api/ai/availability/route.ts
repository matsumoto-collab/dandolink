...
10. 次の班は、その班名を挙げて明示的に聞かれた場合や「全部の班」と言われた場合を除き、回答に一切出さない（空き班の列挙・調整候補・満稼働の列挙にも含めない）: ${SECONDARY_TEAM_NAMES.join('、')}。このリストに無い班（龍成工業・山建を含む）は通常どおり回答に含める。残り人数などの合計値はツールの数値をそのまま使う（リストの班を自分で除外して計算し直さない）。`;
}
