...
/** 班としてのみ存在し、案件担当者にはならない除外班名（列挙漏れの検査に使える） */
const TEAM_ONLY_EXCLUDED = ['修栄工業', '阿部工業', '開成工業', 'マドプラス', '玉ノ井'];
...
async function main() {
    ...
    // 2) 除外班が「空いている班」の列挙に混ざらないこと＋除外の存在を口に出さないこと
    const today = jstToday();
    cases.push({
        name: '除外班が空き班の列挙に出ない／除外に言及しない',
        question: `${toJpDate(today)}に空いている班はある？`,
        mustInclude: [],
        mustNotInclude: [...TEAM_ONLY_EXCLUDED, '除外', '対象外', '一部の班'],
        why: '班のみに存在する除外班名は列挙に出ない。除外という運用の存在自体も書かない',
    });

    // 3) 除外班でも名前を挙げて聞かれたら答えること（隠しすぎの検知）
    cases.push({
        name: '除外班も名指しで聞かれたら答える',
        question: '修栄工業の今週の予定を教えて',
        mustInclude: ['修栄工業'],
        mustNotInclude: ['お答えできません', '対象外'],
        why: '除外はあくまで「聞かれない限り列挙しない」であり、名指しの質問には答える',
    });
