...
export default function ProjectMasterDetailModal({ pm, onClose, onUpdate, initialEditMode }: ProjectMasterDetailModalProps) {
    ...
    return (
        <div className="fixed inset-0 lg:left-64 z-[60] flex items-start pt-[4.5rem] lg:items-center lg:pt-0 justify-center overflow-y-auto">
            ...
            >
                {/* ヘッダー */}
                <div className={`sticky top-0 border-b border-gray-200 px-4 md:px-6 py-4 flex items-center justify-between z-10 transition-colors ${isEditMode ? 'bg-amber-50' : 'bg-white'}`}>
                    <div className="flex-1 min-w-0 mr-3">
                        <div className="flex items-center gap-2 flex-wrap">
                            <h2 className="text-lg md:text-xl font-semibold text-gray-900 truncate">{pm.title}</h2>
                            {isEditMode && (
                                <span className="px-2 py-0.5 text-xs font-bold bg-amber-200 text-amber-800 rounded-full whitespace-nowrap">
                                    編集中
                                </span>
                            )}
                        </div>
