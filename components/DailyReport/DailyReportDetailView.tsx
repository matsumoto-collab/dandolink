    ...
    useEffect(() => {
        const fetchWorkers = async () => {
            try {
                ...
                if (res.ok) {
                    const workers: { id: string; displayName: string }[] = await res.json();
                    setWorkerNameMap(new Map(workers.map(w => [w.id, w.displayName])));
                }
            } catch (e) { /* ignore */ }
        };
        fetchWorkers();
        ...
        <div className="space-y-6">
                ...
                {sortedWorkItems.length === 0 ? (
                    ...
                    <div className="space-y-2">
                        {sortedWorkItems.map((item) => {
                            ...
                            return (
                                <div key={item.assignmentId} className="p-3 bg-slate-50 rounded-lg">
                                    ...
                                    <div className="text-sm text-slate-600 mt-1">
                                        ...
                                        {(item.breakMinutes ?? 0) > 0 && (
                                            <span className="ml-2 text-slate-400">（休憩 {formatMinutes(item.breakMinutes!)}）</span>
                                        )}
                                    </div>
                                    {(() => {
            ...
            <div className="space-y-3 pt-4 border-t border-slate-200">
                ...
                <div className="flex gap-3">
                    <button
                        onClick={onClose}
                        className={`px-4 py-2 border border-slate-300 rounded-md text-slate-700 hover:bg-slate-50 transition-colors font-medium ${canEdit ? 'flex-1' : 'w-full'}`}
                    >
                        閉じる
                    </button>
                    {canEdit && (
