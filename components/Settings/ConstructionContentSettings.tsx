...
export default function ConstructionContentSettings() {
    const [contents, setContents] = useState<ConstructionContent[]>([]);
    ...
    const handleEdit = (item: ConstructionContent) => {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            const res = await fetch(`/api/master-data/construction-contents/${editingId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleDelete = async (id: string) => {
        try {
            const res = await fetch(`/api/master-data/construction-contents/${id}`, {
                method: 'DELETE',
            });

            if (res.ok) {
                toast.success('工事内容を削除しました');
    ...
    return (
        <div className="min-w-0 overflow-hidden">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h3 className="text-lg font-semibold text-slate-900">工事内容一覧</h3>
                    <p className="text-sm text-slate-500 mt-1">案件の工事内容を管理します（例: 新築、改修、大規模）</p>
                </div>
            </div>
