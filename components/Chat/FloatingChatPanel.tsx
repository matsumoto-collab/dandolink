...
import { CHAT_WINDOW_MEDIA_QUERY } from '@/lib/chatWindow';
import ChatRoomView from './ChatRoomView';
import ChatRoomList, { filterRooms, roomTitle } from './ChatRoomList';
import NewRoomModal from './NewRoomModal';

/**
 * チャットウインドウ。PC・iPad ではサイドバーの「チャット」を押すとチャット画面へは行かず、
 * このウインドウで開く（スケジュールを見ながら返信できるようにするため）。
 *  - 幅768px以上(PC・iPad): 自由に移動・リサイズできる画面内ウインドウ
 *  - 幅768px未満(スマホ): 画面下のボトムシート（「予定」からのドッキング用。ヘッダーのタップで展開/折りたたみ）
 *
 * 中身:
 *  - ルーム未選択（dockedRoomId が null）: ルーム一覧（検索・新規チャット）だけを出す
 *  - ルーム選択中: チャット本文。一覧はウインドウ幅 LIST_COLUMN_MIN_WINDOW_W 以上なら左カラムとして
 *    常時表示（ヘッダーのボタンで隠せる）、それ未満・ボトムシートでは自動で折りたたみ、
 *    同じボタンでチャットの上に重ねて出す
 *
...
export default function FloatingChatPanel() {
    ...
    // 職方・協力業者・応援はチャット新規作成不可（ChatPage と同じ）
    ...
    const closeChatWindow = useChatStore((s) => s.closeChatWindow);
    ...
    useEffect(() => {
        const mql = window.matchMedia(CHAT_WINDOW_MEDIA_QUERY);
    ...
    /** ヘッダー左端: チャット一覧の開閉（ルーム選択中のみ。未選択時は本文が一覧）。他ルームの未読はバッジで知らせる */
    const listButton = hasRoom ? (
        <button
            type="button"
            onClick={toggleList}
    ...
    const headerActions = (
        <>
            {/* スマホのボトムシートだけ「チャット画面で開く」を出す（PC・iPad はウインドウが正） */}
            {!isFloating && hasRoom && (
                <button
                    type="button"
    ...
    /** 一覧の中身（検索欄＋新規チャット＋ルーム一覧）。ルーム未選択時の本文・左カラム・重ね表示で共用 */
    const listPanel = (
        <div className="flex flex-col h-full min-h-0 bg-slate-50">
            <div className="flex-shrink-0 flex items-center gap-1.5 p-2 border-b border-slate-200 bg-white">
                ...
                {canCreateRoom && (
                    <button
                        ...
                        className="flex-shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-lg bg-teal-600 hover:bg-teal-700 text-white shadow-sm"
                        aria-label="新規チャット"
                        title="新規チャット"
                    >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto">
                <ChatRoomList
                    ...
                    emptyMessage={canCreateRoom ? <>チャットがありません<br />「+」から新規作成</> : 'チャットがありません'}
                    compact
                />
            </div>
        </div>
    ...
    /** 本文。displayClass は 'flex' か 'hidden'（ボトムシート折りたたみ中） */
    const renderBody = (displayClass: string) => (
        <div className={`flex-1 min-h-0 relative ${displayClass}`}>
            {!hasRoom ? (
                // ルーム未選択: 一覧だけ
                <div className="flex-1 min-w-0 min-h-0">{listPanel}</div>
            ) : (
                <>
                    {showColumn && (
                        <aside
    ...
    /** 新規チャット作成。作成したらそのルームをウインドウで開く（ウインドウ枠の外に描く＝fixed の基準を確実に画面にする） */
    const newRoomModal = showNewRoomModal ? (
        <NewRoomModal
