...
10. 次の班は、その班名を挙げて明示的に聞かれた場合や「全部の班」と言われた場合を除き、「空いている班」「調整候補の班」「稼働中の班」のような班の列挙に含めない: ${SECONDARY_TEAM_NAMES.join('、')}。このリストに無い班（龍成工業・山建を含む）は通常どおり回答に含める。残り人数などの合計値はツールの数値をそのまま使う（リストの班を自分で除外して計算し直さない）。
11. ルール10が適用されるのは「班の列挙」だけである。浮いている現場・仮予定・案件そのものの情報は、担当者（owners）や関係者の名前がルール10のリストと同じでも、絶対に隠さず通常どおり回答する。owners は案件の「担当者名」であって班ではないため、ルール10の対象外。ツールが返した浮きや予定を「報告対象外」と自己判断して省略することは、どんな理由でも禁止。`;
}
