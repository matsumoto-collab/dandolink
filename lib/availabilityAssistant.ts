...
const TOOLS: Anthropic.Tool[] = [
    ...
    },
    {
        ...
        description:
            '指定した地名の「近く」で予定が入っている現場を、近い順に取得する（現調でまとめて回るための照会）。各現場の distanceKm（基準点からの直線距離）・日付・班名（team=null は浮き＝班未定）・住所・人数・確定/仮・担当者を返す。既定の半径は3kmで、1件も無ければ自動で5kmまで広げる（そのとき expanded=true・radiusKm=5）。notice はそのまま回答に含める注記。resolved が null のときはその地名の場所を特定できていない。unknownLocation は住所（座標）が未登録で距離を判定できなかった案件。jobs の outsideRadius=true は5km以内にも1件も無かったときの「一番近い現場」。',
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                radiusKm: { type: 'number', description: '半径km（最大50）。質問で「◯km以内」と明示されたときだけ渡す。渡さなければ3km→空振りなら5kmの自動判定になる' },
            },
            required: ['place'],
        },
    },
...
- 「◯◯（地名）の近く」「◯◯周辺」「◯◯のあたり」= find_nearby_jobs で地名の周辺の予定を探す（既定は半径3km＝現調でついでに寄れる範囲。1件も無ければツールが自動で5kmまで広げる）。「◯km以内」と明示されたときだけ radiusKm を渡す。期間の指定がなければ今日から7日間。
...
11. 距離は必ず find_nearby_jobs の distanceKm を「約◯km」と言う。地図の知識で距離や位置関係を推測してはいけない。jobs は現場ごとにまとまっていて schedule にその現場の予定日が入っているので、1現場1行で「・約2km 松山市北条辻 山田様邸 7/24(金)・7/25(土) 玉ノ井班」のように、距離・住所・現場名・日付・班名を並べる。schedule の team が null の日は班名の代わりに「浮き（班未定）」、dateStatus が tentative の日は日付に「(仮)」を付ける。近い順に並んでいる順番を変えない。**jobs に入っている現場は1件残らず挙げる。多いからといって自分の判断で省略しない**（どれに寄るかを決めるのは聞いた人であって、あなたではない）。
...
13. resolved が null のときは、場所を推測せず「◯◯がどのあたりか分かりませんでした。市区町村名を付けて聞いてください」と答える。結果に notice が入っていたら、その文を一覧の前か後ろにそのまま入れる（言い換えても良いが、意味は落とさない）。
