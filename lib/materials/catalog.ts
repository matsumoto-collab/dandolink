...
/**
 * シート（ネット）の種類固定定数。7 種（出庫伝票のシート箱ドロップダウン）。
 * 現行 seed-materials.ts の「新素用」は誤字。ここでは「新築用」に修正して定義する。
 * （PDF 側の spec ラベルも「新築用 青(紐付) 1.8」表記であり、こちらが正）
 *
 * 2026-06-25 kei 指示で内容変更:
 *   追加: '新築用 グレー(紐付)'（'新築用 青(紐付)' とペア・半角括弧で表記統一）/ '黄'
 *   削除: 'グレー6.3' / '青'
 * ※ ここはシート箱で選べる「種類」の一覧。在庫品目（seed-materials.ts の 'ネット'
 *    カテゴリ）とは別軸なので、在庫側は変更しない。
 */
